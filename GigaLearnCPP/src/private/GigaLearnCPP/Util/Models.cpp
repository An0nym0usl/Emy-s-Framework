...
torch::Tensor GGL::Model::Forward(torch::Tensor input, bool halfPrec) {
	...
	// Half/bf16 Infer is a CUDA win; on CPU (Win AMD PPO + Collect Infer) it is
	// software-emulated and tanks Collection/Overall SPS. Keep fp32 on host.
	if (torch::GradMode::is_enabled() || !input.is_cuda() || !device.is_cuda())
		halfPrec = false;
