...
// Forward declaration so the GPU bridge can be declared without pulling in the RocketSimCuda header here.
namespace rsc { class RocketSimCudaBatch; }
#endif
...
#ifdef GIGA_USE_CUDA_SIM
		// --- Hybrid GPU backend bridge (RocketSimCuda) ---
		// Populates this GameState from one arena of a GPU RocketSimCudaBatch (state read back from
		// the device), so the existing CPU observation/reward/terminal code can run unchanged on top
		// of GPU-simulated physics. Mirrors UpdateFromArena field-for-field.
		explicit GameState(rsc::RocketSimCudaBatch& batch, int arenaIdx);
		void UpdateFromCudaBatch(rsc::RocketSimCudaBatch& batch, int arenaIdx,
